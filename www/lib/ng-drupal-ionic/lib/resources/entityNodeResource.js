...
EntityNodeResourceModules.service('EntityNodeResource', [ 'drupalApiConfig', 'BaseResource', 'EntityNodeResourceConfig', 'EntityNodeResourceChannel', '$http', '$q', 
	 ...
	 * 
	 */
	var update = function( nid, entity_node, options ) {
		...
		if(!nid) { errors.push('Param nid is required.'); }
		//if not given
		if(!entity_node) { errors.push('Param entity_node is required.'); }
		//if is not an array
		if( entity_node instanceof Array ) { errors.push('Param entity_node has to be an array.');}
	...
	/*
	 * Files
	 * Drupal CORS settings api_endpoint/entity_node/files/*|<mirror>|GET|Content-Type
	 * 
	...
	/*
	 * Attach files
	 * Drupal CORS settings api_endpoint/entity_node/attach_file*|<mirror>|GET|Content-Type
	 * 
	 * Upload and attach file(s) to a node. POST multipart/form-data to node/123/attach_file
	 * 
	 * Method: POST 
	 * Url: http://drupal_instance/api_endpoint/entity_node/{NID}/attach_file/
	 * Headers: Content-Type:application/json
	 * 
	 * @param {Integer} nid The nid of the entity_node to attach a file to, required:true, source:path
	 * @param {Sting} field_name The file field name, required:true, source:post body
	 * @param {Integer} attach Attach the file(s) to the entity_node. If FALSE, this clears ALL files attached, and attaches the files, required:false, source:post body
	...
	var attach_file = function(nid, field_name, attach, field_values) {
		var attachFilePath = drupalApiConfig.drupal_instance + drupalApiConfig.api_endpoint + EntityNodeResourceConfig.resourcePath + '/' + nid + '/' + EntityNodeResourceConfig.actions.attachFile,
			defer = $q.defer(),
			requestConfig = {
		...
		if(errors.length != 0) {
			EntityNodeResourceChannel.publishEntityNodeAttachFileFailed(errors);
			defer.reject(errors); 
			return defer.promise;
		};
		...
		.success(function(data, status, headers, config){
			EntityNodeResourceChannel.publishEntityNodeAttachFileConfirmed(data);
			defer.resolve(data);
		})
		.error(function(data, status, headers, config){
			EntityNodeResourceChannel.publishEntityNodeAttachFileFailed(data);
			defer.reject(data);
		});
